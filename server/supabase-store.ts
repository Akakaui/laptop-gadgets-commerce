...
async function loadRemote(): Promise<Store> {
  ...
  return {
    products: (products.data || []).map((row: any) => ({ id: row.id, name: row.name, brand: row.brand, category: row.category || undefined, price: Number(row.price), compareAt: row.compare_at ? Number(row.compare_at) : undefined, stock: row.stock, limited: row.limited, condition: row.condition, badge: row.badge || undefined, description: row.description || undefined, specs: row.specs || [], colors: row.colors || [], imageTone: row.image_tone || 'carbon', imageUrl: row.image_url || undefined, sku: row.sku || undefined, weight: row.weight || undefined, sizes: row.sizes || [], variants: row.variants || [], tags: row.tags || [], rating: Number(row.rating || 0), reviewCount: Number(row.review_count || 0) })),
...
async function persistRemote(store: Store) {
  ...
  const products = store.products.map((product: Product) => ({ id: product.id, name: product.name, slug: slugify(product.name), brand: product.brand || null, category: product.category || null, condition: product.condition || null, description: product.description || null, price: product.price, compare_at: product.compareAt || null, stock: product.stock, limited: product.limited, badge: product.badge || null, specs: product.specs, colors: product.colors || [], image_tone: product.imageTone || 'carbon', image_url: product.imageUrl || null, sku: product.sku || null, weight: product.weight || null, sizes: product.sizes || [], variants: product.variants || [], tags: product.tags || [], rating: product.rating || 0, review_count: product.reviewCount || 0, active: true, updated_at: new Date().toISOString() }));
